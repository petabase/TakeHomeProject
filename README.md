...
│   └── README.md
├── testcase-generator/
│   ├── extract_real_labels.py     # pulls real label images + ground truth from TTB's
│   │                               #   own official labeling guide PDF (see docs/)
│   ├── generate_synthetic_labels.py # programmatic label generator, full control over
│   │                               #   field values and defect types
│   └── README.md                  # when to use which generator
├── testcase/                      # ready-to-upload test data + expected results,
│   │                               #   produced by testcase-generator/ (see testcase/README.md)
│   ├── SingleLabel/
│   │   ├── WheatBeer.png
...
│   └── malt-beverage-example-labels.pdf  # TTB's official label examples guide (source
│                                   #   data for testcase-generator/extract_real_labels.py)
...
Test data comes from two sources, generated by the scripts in [`testcase-generator/`](testcase-generator/README.md):

- **Real TTB label examples**, extracted directly from TTB's own official guide, [*Malt Beverage Label Examples* (TTB G 2023-9)](https://www.ttb.gov/system/files/images/beer/labeling/malt-beverage-example-labels.pdf) — archived at [`docs/malt-beverage-example-labels.pdf`](docs/malt-beverage-example-labels.pdf). These are TTB's own published compliant/non-compliant pairs, with the agency's own stated reasoning for each rejection. A subset has deliberate angle, blur, or glare distortion applied to test image-quality tolerance alongside field-matching correctness.
- **Programmatically generated labels**, for fast, fully-controllable test cases at volume.

Ready-to-upload test cases — a single-label example and a 14-image batch, both with expected/ground-truth results — live in [`testcase/`](testcase/README.md), along with step-by-step instructions for running each one through the app.
